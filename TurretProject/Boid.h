...
{
public:
	const int FLOCK = 0, SWARM = 1, FACTORY = 2, PREDS = 3;
	bool predator;
	Pvector location;
	Pvector velocity;
...
*/	
	...
	Pvector Separation(vector<Boid> Boids, sf::Vector2f playerPos);
	Pvector AsteroidSeparation(vector<Boid> Boids, sf::Vector2f asteroidPos);
	Pvector Alignment(vector<Boid> Boids);
	Pvector Cohesion(vector<Boid> Boids);
	//Functions involving SFML and visualisation linking
	Pvector seek(Pvector v);
	void run(vector <Boid> v, sf::Vector2f playerPos, sf::Vector2f asteroidPos);
	void update();
	void flock(vector <Boid> v, sf::Vector2f playerPos);
	void AvoidAsteroids(vector <Boid> v, sf::Vector2f asteroidPos);
	void borders();
	int getType() { return boidType; };
	float angle(Pvector v);
	tuple<bool, Pvector> CalcLJ(Boid b, sf::Vector2f playerPos);
	void swarm(vector <Boid> v, sf::Vector2f playerPos);
	Pvector Boid::Separation(sf::Vector2f playerPos);
